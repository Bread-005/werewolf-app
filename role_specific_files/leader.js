...
async function leaderAction(nightPhaseText) {
    ...
    if (!storage.leaderKnowsEverything) {
        ...
        if (storage.activatedRoles.find(role => role.name === "Vampire" || role.name === "Master" || role.name === "Count")) {
            nightPhaseText.textContent = "Vampire zeigt in die Richtung des Spielers mit der Vampir-Marke.";
            await speak("./voices/leader/vampire_text.mp3");
        }
    }

    if (storage.leaderKnowsEverything) {
        nightPhaseText.textContent = "Schaue dir alle Karten an.";
        await speak("./voices/leader/all_knowing_text.mp3");
    }
}
